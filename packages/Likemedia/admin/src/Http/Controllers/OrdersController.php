...
class OrdersController extends Controller
{
    ...
    public function getOrdersList(Request $request)
    {
        ...
        if ($request->get('status')) {
            if ($request->get('status') == 'preorders') {
                $orders = CRMOrders::where('main_status', '!=', 'preorders')->where('user_id', '!=', '0')->orderBy('id', 'desc')->get();
            }else{
                $orders = CRMOrders::where('main_status', $request->get('status'))->where('user_id', '!=', '0')->orderBy('id', 'desc')->get();
            }
        }else{
            $orders = CRMOrders::where('main_status', 'preorders')->where('user_id', '!=', '0')->orderBy('id', 'desc')->get();
        }
    ...
    public function getOrdersListGuests(Request $request)
    {
        ...
        if ($request->get('status')) {
            if ($request->get('status') == 'preorders') {
                $orders = CRMOrders::where('main_status', 'preorders')->where('guest_user_id', '!=', '0')->orderBy('id', 'desc')->get();
            }else{
