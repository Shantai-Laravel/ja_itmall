...
class AutoUploadController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('admin::admin.autoupload.index', compact('categories', 'currentCategory', 'products', 'promotions', 'sets', 'collections', 'brands'));
    }
    ...
    public function getMaterials(Request $request)
    {
        ...
        if (!is_null($productMaterialValue)) {
            ...
            $data['materials'] = Product::with('translation')->whereIn('id', $materialsId)->where('id', '!=', $productId)->get();

            $data['checkedMaterials'] = ProductMaterial::where('product_id', $productId)->whereIn('material_id', $materialsId)->pluck('material_id')->toArray();
