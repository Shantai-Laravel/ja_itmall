    ...
    <tr class="row100">
        ...
        <td class="column-tenumberxt">
            ...
            <div class="form-check">
                ...
                <label class="form-check-label" :for="'.com' + product.id">.com</label>
            </div>
            <div class="form-check">
        ...
        <td class="column-select" v-for="param in category.params" v-if="dependeble !== param.property.id">
            <span v-if="param.property.type === 'select'"><i>{{ strCut(product.translations[0].name) }}</i></span>
            <select @change="setChange" v-model="properties[param.property.id]" v-if="param.property.type === 'select' || param.property.type === 'checkbox'">
    	...
    	<!-- <td class="column-number">
            <span><i>{{ strCut(product.translations[0].name) }}</i></span>
            <input type="number" v-model="price=product.price" placeholder="-" @keyup="setChange">
        </td> -->
    	<td class="column-number">
            <span><i>{{ strCut(product.translations[0].name) }}</i></span>
        ...
        <!-- <td class="column-button">
            <span><i>{{ strCut(product.translations[0].name) }}</i></span>
            <button type="button" class="btn btn-default btn-sm" data-toggle="modal" :data-target="'#material-modal' + product.id" @click="getMaterials()"><i class="fa fa-tag"></i> Materials <span class="badge">{{ materials.length }}</span></button>
        </td> -->
        <td class="column-button">
            <span><i>{{ strCut(product.translations[0].name) }}</i></span>
        ...
        <div class="modal fade bd-example-modal-lg settings-modal" :id="'material-modal' + product.id" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                ...
                <div class="modal-content">
                    <div class="row">
                        <div class="col-md-12">
                            <h5 class="text-center"><b>Materials : </b> {{ product.translations[0].name }}</h5>
                            <hr>
                        </div>
                        <div class="col-md-12">
...
<script>
    ...
    export default {
        props: ['category', 'index', 'prod', 'langs', 'promotions', 'sets', 'collections', 'brands', 'categories'],
        data(){
            return {
                product: this.prod,
                name : [],
        ...
        methods: {
            ...
            save(){
                ...
                axios.post('/back/auto-upload-edit', {product: this.product, properties: this.properties, propertiesText: this.propertiesText})
                    .then(response => {
                        bus.$emit('endLoading');
                    })
                    .catch(e => {
                ...
                let defaultVal = [];
                 ...
                 this.category.params.forEach(function (entry) {
                     if (vm.dependeble !== entry.property.id) {
                         defaultVal[entry.parameter_id] = vm.getProductValue(entry.parameter_id);
                     }
                 })
                 this.properties = defaultVal;
            },
            getProductValue(propId){ //get product value
                let ret = 0;
