...
class ProductsController extends Controller
{
    ...
    public function productRender($category, $product)
    {
        ...
        if (count($productMaterial) > 0) {
            $similarProducts = Product::whereIn('id', $productMaterial)->get();
        }else{
        ...
        $wearWith = Product::where('hit', 1)->whereIn('id', $allProds)->where('id', '!=', $product->id)->limit(12)->get();
        ...
        if ($product->category->product_type === "pre-order") {
            return view('front.products.productPreOrder', compact('seoData', 'category', 'product', 'similarProducts'));
        }else{
